...
// (function(){
	...
	var hostPath;
	var urlPath;
	var urlWhiteList = ['baidu.com','google.com','github.com'];
	...
	function parameter_Xss(){ //URL参数检测XSS
		...
		for(i = 0;i < parameter.length;i++){
			for(j=0;j< payloads.length;j++){ //get 参数加入多个参数的fuzz
				var parameterData = parameter[i]; //保存一下原来的参数
				parameter[i] = parameter[i].split("=")[0] + "=" + parameter[i].split("=")[1] + payloads[j];
				$.ajax({
	...
	function pseudoStatic_Xss(){	//伪静态检测XSS
		...
		if(urlPath[urlPath.length-1].indexOf(".") != "-1"){
			fileURL = urlPath.pop();
			fileUrlXss = fileURL.split(".")[0] + onlyString + "." + fileURL.split(".")[1] //会检测xss"><img>.php 这样的页面
			$.ajax({
